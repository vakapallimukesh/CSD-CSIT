...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($username) || empty($password)) {
        ...
        // Check Super Admin
        if (($username === 'superadmin' || $username === 'superadmin@srkrec.edu.in') && $password === 'super123admin') {
            $_SESSION['superadmin_logged_in'] = true;
            $_SESSION['superadmin_username'] = 'superadmin';
            $_SESSION['username'] = 'superadmin';
            $_SESSION['last_activity'] = time();
            $_SESSION['expire_time'] = 1200;
            header("Location: admin/pages/super_admin_dashboard.php");
            exit();
        }

        // Check House Admin credentials (Agni, Prudhvi, Vayu, Aakash, Jal)
        $admin_stmt = mysqli_prepare($conn, "SELECT admin_id, username, password, hid FROM admins WHERE LOWER(username) = LOWER(?) OR LOWER(username) = LOWER(CONCAT(?, '_admin'))");
        if ($admin_stmt) {
            $clean_user = str_replace(['_admin', '@srkrec.edu.in'], '', strtolower($username));
            mysqli_stmt_bind_param($admin_stmt, "ss", $username, $clean_user);
            ...
            if ($admin_row = mysqli_fetch_assoc($admin_result)) {
                if ($password === $admin_row['password']) {
                    ...
                    $_SESSION['admin_hid'] = $admin_row['hid'];
                    $_SESSION['last_activity'] = time();
                    $_SESSION['expire_time'] = 1200;
...
<body>
    <div class="login-container">
        ...
        <div class="login-body">
            ...
            <div class="user-type-badges">
                <button type="button" class="user-badge" id="hodLoginBtn" data-role="hod"><i class="fas fa-user-shield me-1"></i> HOD</button>
                <button type="button" class="user-badge" id="facultyLoginBtn" data-role="faculty"><i class="fas fa-chalkboard-teacher me-1"></i> Faculty</button>
                <button type="button" class="user-badge" id="houseAdminBtn" data-role="house_admin"><i class="fas fa-user-gear me-1"></i> House Admin</button>
    ...
    <script>
        ...
        // Interactive User Role Selection Buttons (HOD, Faculty, Student)
        document.querySelectorAll('.user-badge').forEach(btn => {
            btn.addEventListener('click', function(e) {
                ...
                if (role === 'hod') {
                    usernameEl.value = 'hod@srkrec.edu.in';
                    passwordEl.value = 'hod123';
                } else if (role === 'faculty') {
                    usernameEl.value = 'sureshbabu.k@srkrec.edu.in';
                    passwordEl.value = 'faculty123';
                } else if (role === 'house_admin') {
                    usernameEl.value = '';
                    passwordEl.value = '';
                    usernameEl.placeholder = 'Enter House Admin Username (e.g. agni_admin)';
                    usernameEl.focus();
                } else if (role === 'student') {
                    usernameEl.value = '';
                    passwordEl.value = '';
                    usernameEl.placeholder = 'Enter student email or ID';
                    usernameEl.focus();
                }
                
                // Update input styling states
                document.querySelectorAll('.form-control').forEach(input => {
                    if (input.value.trim() !== '') {
                        input.parentElement.parentElement.classList.add('filled');
                    } else {
