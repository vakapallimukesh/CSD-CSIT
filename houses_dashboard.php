...
// PHP 8.1+ throws mysqli_sql_exception by default — disable so missing
// tables/columns don't crash the page (graceful fallback with defaults)
...
// Calculate statistics dynamically from database if available
$house_stats = [];
foreach ($houses as $house_key => $house_info) {
    ...
    if (isset($conn) && $conn) {
        // Find house ID (hid)
        $hid = null;
        $house_name_escaped = mysqli_real_escape_string($conn, $house_name);
        $house_sql = "SELECT hid FROM houses WHERE name = '$house_name_escaped' OR UPPER(name) = UPPER('$house_name_escaped')";
...
if (isset($conn) && $conn) {
    try {
        ...
        if ($events_result) {
            ...
            $db_events = (int)($events_data['total'] ?? 0);
            if ($db_events > 0) {
                $overall_stats['total_events'] = $db_events;
            }
        }
    } catch (Exception $e) {
        // Table or column missing — use defaults
    }
}
...
.house-card-Aakash:hover .house-overlay,
.house-card-Agni:hover .house-overlay,
.house-card-Jal:hover .house-overlay,
.house-card-PRUDHVI:hover .house-overlay,
.house-card-Pruthvi:hover .house-overlay,
.house-card-Vayu:hover .house-overlay,
.house-card:hover .house-overlay,
.house-link:hover .house-overlay {
    opacity: 1 !important;
...
<body style="background: #fdfbf7; min-height: 100vh; font-family: 'Plus Jakarta Sans', 'Inter', sans-serif; color: #1a0d06;">
                    ...
                    <div class="col-xl-2 col-lg-3 col-md-4 col-sm-6 mb-3" style="width:260px">
                        <a href="house_detail.php?house=${encodeURIComponent(houseKey)}" class="text-decoration-none house-link">
                            <div class="house-card house-card-${cleanKey}" data-house="${houseKey}" style="cursor: pointer;">
                                ...
                                <div class="house-overlay" style="position: absolute; top: 0; left: 0; right: 0; bottom: 0; background: ${overlayBg}; opacity: 0; transition: all 0.3s ease; border-radius: 20px; display: flex; align-items: center; justify-content: center; cursor: pointer;">
                                    <div style="color: white; text-align: center;">
                                        <i class="fas fa-eye" style="font-size: 2rem; margin-bottom: 8px;"></i>
                                        <div style="font-size: 1rem; font-weight: 600;">View Details</div>
                                    </div>
    ...
    <style>
        ...
        /* Let clicks and hovers pass through the overlay so the a-tag and house-card get them */
        .house-overlay {
            border-radius: 20px !important;
